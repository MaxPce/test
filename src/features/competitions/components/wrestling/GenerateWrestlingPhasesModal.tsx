// src/features/competitions/components/wrestling/GenerateWrestlingPhasesModal.tsx
...
import { useGenerateWrestlingPhases } from '@/features/competitions/api/wrestling-phases.mutations';
import type { WrestlingPhaseFormat } from '@/features/competitions/api/wrestling-phases.api';
...
interface WrestlingAthlete {
  registrationId: number;
  name: string;
  institution?: string | null;
}

interface WrestlingPhaseGroup {
  key: string;
  phaseName: string;
  idniv: string;
  idcat: string;
  format: WrestlingPhaseFormat;
  athletes: WrestlingAthlete[];
}
...
export interface GenerateWrestlingPhasesModalProps {
  open: boolean;
  onClose: () => void;
...
export function GenerateWrestlingPhasesModal({
  open,
  onClose,
  eventCategoryId,
  categoryName,
  ...
  sismasterSportId,
  allRegistrations,
}: GenerateWrestlingPhasesModalProps) {
  const mutation = useGenerateWrestlingPhases();
  ...
  const regMap = useMemo(() => {
    const map = new Map<number, WrestlingAthlete>();
  ...
  const initialGroups = useMemo((): WrestlingPhaseGroup[] => {
    ...
    return combos
      .map((combo, idx) => {
        ...
        const athletes = ids
          ...
          .filter((a): a is WrestlingAthlete => {
            if (!a) return false;
        ...
        return {
          ...
          idniv: combo.idniv,
          idcat: combo.idcat,
          format: 'single_elimination' as WrestlingPhaseFormat,
          athletes,
        };
      })
  ...
  const fallbackGroup = useMemo((): WrestlingPhaseGroup[] => {
    if (hasSismaster || !open) return [];
    const athletes: WrestlingAthlete[] = allRegistrations.map((reg) => ({
      registrationId: reg.registrationId,
      name: reg.athlete?.name ?? reg.team?.name ?? `Registro ${reg.registrationId}`,
      institution: reg.athlete?.institution?.name ?? reg.team?.institution?.name ?? null,
    }));
  ...
  const [groups, setGroups] = useState<WrestlingPhaseGroup[]>([]);
  ...
  const initialGroupsRef = useRef<WrestlingPhaseGroup[]>([]);
  ...
  const setGroupFormat = (key: string, format: WrestlingPhaseFormat) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error('Error generando fases de lucha olímpica:', err?.response?.data);
    }
  };
