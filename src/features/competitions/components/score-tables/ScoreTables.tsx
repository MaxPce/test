// src/components/score-tables/ScoreTables.tsx
...
/** Skeleton de una fila de la tabla */
function SkeletonRow() {
  return (
    <tr className="animate-pulse">
      <td className="px-4 py-3">
        <div className="h-4 w-6 rounded bg-orange-100" />
      </td>
      <td className="px-4 py-3">
...
function RankBadge({ rank }: { rank: number }) {
  if (rank === 1) {
    return (
      <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-amber-400 text-xs font-bold text-white shadow-sm">
        1
      </span>
    );
  }
    ...
    >
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span
            className={[
              'text-sm',
              isFirst ? 'font-semibold text-orange-700' : 'text-gray-700',
            ].join(' ')}
          >
            {row.institutionName}
          </span>
      ...
      <td className="px-4 py-3 text-center">
        <span
          className={[
            ...
            isFirst ? 'text-orange-600' : 'text-gray-800',
          ].join(' ')}
        >
          {row.points}
        </span>
      ...
      <td className="px-4 py-3 text-center">
        <span className="inline-flex items-center gap-1 text-sm tabular-nums">
          <span aria-label="Oro">🥇</span>
          <span className="font-medium text-gray-700">{row.gold}</span>
        </span>
      </td>
      <td className="px-4 py-3 text-center">
        <span className="inline-flex items-center gap-1 text-sm tabular-nums">
          <span aria-label="Plata">🥈</span>
          <span className="font-medium text-gray-700">{row.silver}</span>
        </span>
      </td>
      <td className="px-4 py-3 text-center">
        <span className="inline-flex items-center gap-1 text-sm tabular-nums">
          <span aria-label="Bronce">🥉</span>
          <span className="font-medium text-gray-700">{row.bronze}</span>
        </span>
      </td>
    </tr>
  );
...
export function ScoreTables({ eventId }: ScoreTablesProps) {
  const [activeTab, setActiveTab] = useState<ScoreTableKey>('general');
  const { data, loading, error, refresh } = useScoreTables(eventId);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-400">
            Puntaje oficial FEDUP · 10-8-6-5-4-3-2-1 (postas: doble)
          </p>
        </div>
