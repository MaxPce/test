// src/features/competitions/components/shooting/GenerateShootingPhasesModal.tsx
...
import { AlertCircle, ArrowRightLeft, CheckSquare, Square } from "lucide-react"; // ← ArrowRightLeft agregado
import { useGenerateShootingPhases } from "../../api/shooting.mutations";
...
interface ShootingAthlete {
  registrationId: number;
  name: string;
  institution?: string | null;
}

interface ShootingPhaseGroup {
  key: string;
  phaseName: string;
  idniv: string;
  idcat: string;
  athletes: ShootingAthlete[];
}

export interface GenerateShootingPhasesModalProps {
  open: boolean;
  onClose: () => void;
...
// ← FIX 1: phaseName ahora incluye el nombre de la categoría
...
export function GenerateShootingPhasesModal({
  open,
  onClose,
  eventCategoryId,
  categoryName = "Tiro Deportivo",
  sismasterEventId,
  sismasterSportId,
  allRegistrations,
}: GenerateShootingPhasesModalProps) {
  const mutation = useGenerateShootingPhases();
  ...
  const regMap = useMemo(() => {
    const map = new Map<number, ShootingAthlete>();
  ...
  const initialGroups = useMemo((): ShootingPhaseGroup[] => {
    ...
    return combos
      .map((combo, idx) => {
        ...
        return {
          ...
          phaseName: buildPhaseName(combo.idniv, combo.idcat, categoryName), // ← categoryName pasado
          idniv: combo.idniv,
          idcat: combo.idcat,
          athletes,
        };
  ...
  // ── Fallback sin Sismaster ────────────────────────────────────────────────
  const fallbackGroup = useMemo((): ShootingPhaseGroup[] => {
    if (hasSismaster || !open) return [];
    const athletes: ShootingAthlete[] = allRegistrations.map((reg) => ({
      registrationId: reg.registrationId,
      name: reg.athlete?.name ?? reg.team?.name ?? `Registro ${reg.registrationId}`,
      institution: reg.athlete?.institution?.name ?? reg.team?.institution?.name ?? null,
    }));
    if (athletes.length === 0) return [];
    return [{ key: "shooting-group", phaseName: categoryName, idniv: "", idcat: "", athletes }];
  }, [hasSismaster, open, allRegistrations, categoryName]);

  // ── Estado editable ───────────────────────────────────────────────────────
  const [groups, setGroups] = useState<ShootingPhaseGroup[]>([]);
  const [selectedGroupKeys, setSelectedGroupKeys] = useState<Set<string>>(new Set());
  // ← FIX 2: estado para mover atletas
  const [selectedRegId, setSelectedRegId] = useState<number | null>(null);
  const [sourceGroupKey, setSourceGroupKey] = useState<string | null>(null);

  const hasInitialized = useRef(false);
  const prevOpenRef = useRef(false);
  const initialGroupsRef = useRef<ShootingPhaseGroup[]>([]);
  ...
  useEffect(() => {
    if (!open && prevOpenRef.current) {
      ...
      setSelectedRegId(null);   // ← reset al cerrar
      setSourceGroupKey(null);  // ← reset al cerrar
    }
  ...
  const toggleSelectAll = () => {
    if (allSelected) setSelectedGroupKeys(new Set());
    else setSelectedGroupKeys(new Set(groups.map((g) => g.key)));
  };

  // ── Mover atleta entre fases ──────────────────────────────────────────────
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error("Error al generar fases:", err?.response?.data);
    }
  };
      ...
      <div className="space-y-4">
        ...
        {hasSismaster && !isLoading && !combosError && allQueriesDone && groups.length === 0 && (
          <p className="py-10 text-center text-sm text-slate-500">
            No se encontraron tiradores inscritos con nivel/categoría en Sismaster.
          </p>
        )}

        {/* Sin tiradores (sin Sismaster) */}
        {!hasSismaster && !isLoading && groups.length === 0 && (
          <p className="py-10 text-center text-sm text-slate-500">
            No hay tiradores registrados en esta categoría.
          </p>
        )}
                      ...
                      <div className="flex items-start gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-xs text-slate-500">
                            {group.athletes.length} tirador(es)
                          </p>
                        </div>
                      </div>
                      {isTarget && (
              ...
              <>
                ...
                {validGroups.length > 0 && (
                  <p className="text-slate-500">
                    {totalAthletes} tirador(es) en las fases seleccionadas
                  </p>
                )}
              </>
            )}
