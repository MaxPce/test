...
import { WushuTaoluScoreInput } from "./WushuTaoluScoreInput";
...
export const WushuTaoluScoreTable = ({ phaseId }: Props) => {
  ...
  const handleSaveScore = (
    ...
    updateScoreMutation.mutate(
      { participationId, data: { accuracy, presentation } },
      {
        onSuccess: () => {
          setEditingId(null);
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white border border-gray-200 rounded-lg">
        ...
        <tbody className="divide-y divide-gray-200">
          {participants.map((participant: WushuTaoluParticipant) => {
            const isEditing = editingId === participant.participationId;

            if (isEditing) {
              return (
                <tr key={participant.participationId} className="hover:bg-gray-50 transition-colors">
                  {/* Participante */}
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
                      {participant.participantName}
                      {participant.isTeam && (
                        <span className="ml-1 text-xs text-red-600">(Equipo)</span>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-sm text-gray-700">{participant.institution}</div>
                    </div>
                  </td>
                  {/* Input inline ocupa las 4 columnas restantes */}
                  <td colSpan={4} className="px-4 py-3">
                    <WushuTaoluScoreInput
              ...
              <tr key={participant.participationId} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-center">
                  <span className="text-sm font-medium text-gray-900">
                    {participant.accuracy != null ? Number(participant.accuracy).toFixed(1) : "-"}
                  </span>
                </td>
                {/* Presentation */}
                <td className="px-4 py-3 text-center">
                  <span className="text-sm font-medium text-gray-900">
                    {participant.presentation != null ? Number(participant.presentation).toFixed(1) : "-"}
                  </span>
                </td>
                {/* Total */}
                <td className="px-4 py-3 text-center">
                  <span className="text-lg font-bold text-red-600">
                    {participant.total != null ? Number(participant.total).toFixed(1) : "-"}
                  </span>
                </td>
                {/* Acciones */}
                <td className="px-4 py-3 text-center">
