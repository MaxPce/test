// src/features/competitions/components/wushu/GenerateWushuPhasesModal.tsx
...
import { useGenerateWushuPhases } from '@/features/competitions/api/wushu-phases.mutations';
import type { WushuPhaseFormat } from '@/features/competitions/api/wushu-phases.api';
...
interface WushuAthlete {
  registrationId: number;
  name: string;
  institution?: string | null;
}

interface WushuPhaseGroup {
  key: string;
  phaseName: string;
  idniv: string;
  idcat: string;
  format: WushuPhaseFormat;
  athletes: WushuAthlete[];
}
...
export interface GenerateWushuPhasesModalProps {
  open: boolean;
  onClose: () => void;
...
export function GenerateWushuPhasesModal({
  open,
  onClose,
  eventCategoryId,
  categoryName,
  ...
  sismasterSportId,
  allRegistrations,
}: GenerateWushuPhasesModalProps) {
  const mutation = useGenerateWushuPhases();
  ...
  const regMap = useMemo(() => {
    const map = new Map<number, WushuAthlete>();
  ...
  const initialGroups = useMemo((): WushuPhaseGroup[] => {
    ...
    return combos
      .map((combo, idx) => {
        ...
        return {
          ...
          idniv: combo.idniv,
          idcat: combo.idcat,
          format: 'single_elimination' as WushuPhaseFormat,
          athletes,
        };
      })
  ...
  const fallbackGroup = useMemo((): WushuPhaseGroup[] => {
    if (hasSismaster || !open) return [];
    const athletes: WushuAthlete[] = allRegistrations.map((reg) => ({
      registrationId: reg.registrationId,
      name: reg.athlete?.name ?? reg.team?.name ?? `Registro ${reg.registrationId}`,
      institution: reg.athlete?.institution?.name ?? reg.team?.institution?.name ?? null,
    }));
  ...
  const [groups, setGroups] = useState<WushuPhaseGroup[]>([]);
  ...
  const initialGroupsRef = useRef<WushuPhaseGroup[]>([]);
  ...
  const setGroupFormat = (key: string, format: WushuPhaseFormat) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error('Error generando fases de wushu:', err?.response?.data);
    }
  };
