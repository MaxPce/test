...
export const WushuTaoluMatchCard = ({ match }: Props) => {
    ...
    <>
      <Card className="hover:shadow-lg transition-shadow">
        <CardBody className="p-4">
          ...
          {participants.length === 0 ? (
            ...
            <div className="space-y-3">
              {participants.map((participation) => {
                ...
                const name        = reg?.athlete ? reg.athlete.name : reg?.team?.name || "TBD";
                ...
                const accuracy     = (participation as any).accuracy     || 0;
                const presentation = (participation as any).presentation || 0;
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-lg border-2 ${
                      isWinner ? "bg-green-50 border-green-400" : "bg-white border-gray-200"
                    }`}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2 flex-1">
                        {isWinner && <Trophy className="h-4 w-4 text-green-600 flex-shrink-0" />}
                        <div className="flex-1 min-w-0">
                          ...
                          {institution && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <p className="text-xs text-gray-500 truncate">{institution.name}</p>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {(match as any).platformNumber && (
            <p className="text-xs text-gray-600 mt-1">
              Plataforma {(match as any).platformNumber}
            </p>
          )}
        </CardBody>
      </Card>
