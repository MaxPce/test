...
export const WushuMatchDetailsModal = ({ match, phase, isOpen, onClose }: Props) => {
  const advanceWinnerMutation = useAdvanceWinner();
  const [isWushuModalOpen, setIsWushuModalOpen] = useState(false);
    ...
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          {/* Header */}
          <div className="sticky top-0 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-4 flex items-center justify-between z-10 rounded-t-xl">
            ...
            <div className="flex items-center gap-2">
              {canAdvance && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-400 disabled:cursor-not-allowed rounded-lg transition-colors font-medium text-sm backdrop-blur-sm shadow-lg"
                >
              ...
              {canRegisterScore && (
                <button
                  onClick={() => setIsWushuModalOpen(true)}
                ...
                <div className="flex items-start gap-4">
                  {getAthletePhoto(participant1) ? (
                    <img
                      ...
                      className="w-16 h-16 rounded-full object-cover flex-shrink-0 border-4 border-emerald-400"
                      onError={(e) => {
                        ...
                        if (parent) {
                          ...
                          placeholder.className =
                            "w-16 h-16 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center text-white text-2xl font-bold flex-shrink-0";
                ...
                <div className="flex items-start gap-4">
                  {getAthletePhoto(participant2) ? (
                    <img
                      ...
                      className="w-16 h-16 rounded-full object-cover flex-shrink-0 border-4 border-teal-400"
                      onError={(e) => {
                        e.currentTarget.style.display = "none";
                        const parent = e.currentTarget.parentElement;
                        if (parent) {
                          const placeholder = document.createElement("div");
                          placeholder.className =
                            "w-16 h-16 bg-gradient-to-br from-teal-400 to-teal-600 rounded-full flex items-center justify-center text-white text-2xl font-bold flex-shrink-0";
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isWinner(participant2?.participationId || 0) && (
                      <div className="mt-3 inline-flex items-center gap-2 bg-teal-600 text-white px-3 py-1 rounded-full text-sm font-bold">
                        GANADOR
                      </div>
                    )}
                  </div>
      ...
      {canRegisterScore && (
        <WushuScoreModal
          isOpen={isWushuModalOpen}
          onClose={() => setIsWushuModalOpen(false)}
          match={match}
          phase={phase}
        />
      )}
